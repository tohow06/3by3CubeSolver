    ...
    def turn_red():
        ...
        for j in range(5):
            ...
            if f3 >= 5:
                ...
                break
            for i in range(3):
                if color_code[seq1[2][edge[1][1][i][1]-1]] == color_code[seq1[0][edge[1][1][i][0]-1]] or color_code[seq1[2][edge[1][1][i][1]-1]] + color_code[seq1[0][edge[1][1][i][0]-1]] == 7 or color_code[seq1[1][edge[4][1][i][0]-1]] == color_code[seq1[1][edge[4][1][i][1]-1]] or color_code[seq1[1][edge[4][1][i][0]-1]] + color_code[seq1[1][edge[4][1][i][1]-1]] == 7:
                    right_turn(2)
                    f1 = 1
                    break
            if f1 == 1:
...
class RubiksOpenCV(object):
    ...
    def sort_by_row_col(self, contours, size):
        ...
        for row_index in np.arange(size):
    ...
    def set_contour_row_col_index(self, con):
        ...
        row_size = 3 #round(cube_height / float(self.median_square_width + self.black_border_width))
        row_size = min(self.size, row_size)
        median_row_height = float(cube_height / row_size)

        # col_size is how many cols of squares we see
        col_size = 3 #round(cube_width / float(self.median_square_width + self.black_border_width))
    ...
    def analyze(self, webcam, cube_size=None):
        ...
        for con in self.sort_by_row_col(deepcopy(self.candidates), self.size):
            ...
            (mean_blue, mean_green, mean_red, _) = map(int, cv2.mean(hsv, mask = mask))
...
class RubiksVideo(RubiksOpenCV):
    ...
    def draw_circles(self, side_data):
        """
        ...
        if len(self.candidates) >= 10:
            ...
            square = [[0, 0, 0] for i in range(9)]
            for i in range(len(self.candidates)):
                square[i][0] = self.candidates[i].cX
                square[i][1] = self.candidates[i].cY
                square[i][2] = self.candidates[i].width
            x = 5
            y = 5
            num_squares = len(side_data.keys())
            size = int(math.sqrt(num_squares))
            gap = 5
            square_width = 20 #int((self.draw_cube_size - ((size + 1) * gap))/size)
            color_index = {
                'r'    : (0,0,255),
                'o' : (0,165,255),
                'b'   : (255,0,0),
    ...
    def draw_cube_face(self, start_x, start_y, side_data, desc):
        ...
        for (index, square_index) in enumerate(sorted(side_data.keys())):
            ...
            cv2.rectangle((self.image),
                          ...
                          color_index[color_convert],
                          -1)
            appp.append(color_convert)
            if (index + 1) % size == 0:
                y += gap + square_width
                x = start_x + gap
            else:
                x += square_width + gap
        self.writefile(appp, desc)
    ...
    def analyze_webcam(self, width=500, height=500):
        self.video_reset(True)
        ...
        while True:
            ...
            if key == 32:
                flag += 1
                keyss = list(self.data.keys())
                h = self.data[keyss[4]][2]
                s = self.data[keyss[4]][1]
                v = self.data[keyss[4]][0]
                if (s<61 and 140>=h and h>=65) or (v<(s**2-5625)/-22.5) or (s<86 and v<245 and 140>=h and h>=65):
                    self.name = 'D'
                    self.draw_cube_face(self.draw_cube_size, height - self.draw_cube_size - fangg, self.data, 'D')
                elif (s>100 and 115>=h and h>=100 and v<250) or (s>100 and 115>=h and h>=90 and v>250):
                    self.name = 'L'
                    self.draw_cube_face(self.draw_cube_size * 0, height - (self.draw_cube_size * 2) - fangg, self.data, 'L')
                elif (s>100 and 95>=h and h>=70 and v<250) or (s>100 and 90>=h and h>=70 and v>250):
                    self.name = 'R'
                    self.draw_cube_face(self.draw_cube_size * 2, height - (self.draw_cube_size * 2) - fangg, self.data, 'R')
                elif (255>=v and v>110 and 14>=h and h>=4 and s>=75):
                    self.name = 'B'
                    self.draw_cube_face(self.draw_cube_size * 3, height - (self.draw_cube_size * 2) - fangg, self.data, 'B')
                elif (250>v and v>10 and 45>=h and h>=14 and s>=75):
                    self.name = 'U'
                    self.draw_cube_face(self.draw_cube_size, height - (self.draw_cube_size * 3) - fangg, self.data, 'U')
                elif s>0: #(s<150 and v<120):
                    self.name = 'F'
                    self.draw_cube_face(self.draw_cube_size * 1, height - (self.draw_cube_size * 2) - fangg, self.data, 'F')
            if key == ord('b'):
                flag += 1
